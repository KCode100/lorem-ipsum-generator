...
const getInput = () => input = parseInt(inputEl.value);

const displayText = () => {
    loremText.innerHTML = '';
    const newText = text.slice(0, input);
    newText.forEach((par)=> {
        const newPar = document.createElement('p');
        newPar.classList.add('paragraph');
        newPar.textContent = par;
        loremText.appendChild(newPar);
    })
};
...
formEl.addEventListener('submit', generateText);